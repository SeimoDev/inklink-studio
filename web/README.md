...
| `src/device/` | Web Serial、协议解析、传感器、刷新策略和 esptool-js |
...
浏览器从 `public/firmware/inklink-quote0-full.bin` 载入当前一体化固件。文件 URL、版本和 SHA-256 集中定义在 `src/core/release.ts`；更换固件时必须同时更新二进制和校验值。
